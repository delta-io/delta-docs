...
To determine whether a table contains a specific partition, use the following statement. If the partition exists, true is returned.
...
```Python
spark.sql("SELECT COUNT(*) > 0 AS `Partition exists` FROM default.people10m WHERE gender = 'M'").show()
```
...
- If you specify _only the table name and location_, the table in the metastore automatically inherits the schema, partitioning, and table properties of the existing data. This functionality can be used to "import" data into the metastore. For example:
...
- If you specify _any configuration_ (schema, partitioning, or table properties), Delta Lake verifies that the specification exactly matches the configuration of the existing data.
  ...
  <Info title="Note" level="info">
    ...
    table, and the table's transaction log at the location is the source of
    truth. If you query the metastore from a system that is not aware of this
...
<Info title="Important!" level="warning">
  ...
  version than the default. See [Table protocol versioning](/latest/versioning)
...
By default, special characters such as spaces and any of the characters `,;{}()\n\t=` are not supported in table column names. To include these special characters in a table’s column name, enable [column mapping](/latest/versioning/#column-mapping).
...
The data files backing a Delta table are _never_ deleted automatically; data files are deleted only when you run [VACUUM](/latest/delta-utility/#remove-files-no-longer-referenced-by-a-delta-table). `VACUUM` _does not_ delete Delta log files; log files are automatically cleaned up after checkpoints are written.
...
If a partition has been accidentally overwritten, you can use [Restore a Delta table to an earlier state](/latest/delta-utility/#restore-a-delta-table-to-an-earlier-state) to undo the change.

</Info>

For Delta Lake support for updating tables, see [Table deletes, updates, and merges](/latest/delta-update).
...
To rename columns without rewriting any of the columns’ existing data, you must enable column mapping for the table. See [enable column mapping](/latest/versioning/#column-mapping).
...
To drop columns as a metadata-only operation without rewriting any data files, you must enable column mapping for the table. See [enable column mapping](/latest/versioning/#column-mapping).
...
- Configure the [time travel](/latest/delta-batch/#deltatimetravel) retention properties: `delta.logRetentionDuration=<interval-string>` and `delta.deletedFileRetentionDuration=<interval-string>`. For details, see [Data retention](/latest/delta-batch/#data-retention).
...
<Info title="Note" level="info">
Modifying a Delta table property is a write operation that will conflict with other [concurrent write operations](/latest/concurrency-control), causing them to fail. We recommend that you modify a table property only when there are no concurrent write operations on the table.
</Info>
...
Provides information about schema, partitioning, table size, and so on. For details, see [delta details](/latest/delta-utility/#delta-detail).

### `DESCRIBE HISTORY`

Provides provenance information, including the operation, user, and so on, and operation metrics for each write to a table. Table history is retained for 30 days. For details, see [delta history](/latest/delta-utility/#delta-history).
...
You can find the details of the Hadoop file system configurations for your storage in [Storage configuration](/latest/delta-storage).
