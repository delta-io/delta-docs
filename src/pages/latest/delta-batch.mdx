...
The data files backing a Delta table are _never_ deleted automatically; data files are deleted only when you run [VACUUM](/latest/delta-utility/#delta-vacuum). `VACUUM` _does not_ delete Delta log files; log files are automatically cleaned up after checkpoints are written.
...
If a partition has been accidentally overwritten, you can use [TODO: Restore a Delta table to an earlier state](/latest/delta-utility/#restore-a-delta-table-to-an-earlier-state) to undo the change.
