...
- **External readers**: Directly reading the data stored in Delta Lake. For
  information on how to read Delta tables, see [Access Delta tables from external data processing engines](/latest/integrations).
...
For details, see [Convert a Parquet table to a Delta table](/latest/delta-utility#convert-to-delta).
...
new valid column names by using Delta Lake 1.0 and below before upgrading to Delta Lake to 1.1 and above, such as the following:
...
[update](/latest/delta-update) or [run utility operations](/latest/delta-utility) on them,
