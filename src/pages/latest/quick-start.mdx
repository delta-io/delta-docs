...
<Info title="Important" level="info">
  ...
  version of Spark or PySpark that is compatible with Delta Lake `$VERSION$`.
...
```bash
bin/spark-sql --packages io.delta:delta-core_2.12:<compatible-spark-version> --conf "spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension" --conf "spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog"
```
...
[TODO: Write to a table](delta-batch.md#deltadataframewrites).
