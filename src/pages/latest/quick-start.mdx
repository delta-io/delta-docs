...
## Copy paste example 
In this example we install pyspark and delta-spark python packages. Then we start the pyspark shell with the delta packages and configuration. From the python shall we redirect our python example code.

Assumes python 3.10 environment is setup and a bash command shell is open.
```bash
