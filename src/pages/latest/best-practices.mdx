...
This operation does not remove the old files. To remove them, run the [VACUUM](/latest/delta-utility/#remove-files-no-longer-referenced-by-a-delta-table) command.
...
[delete](/latest/delta-update#delete-from-a-table) data from a Delta table and insert
your new data, or [update](/latest/delta-update#update-a-table) the table to fix the
incorrect values.
...
In addition, if you want to delete old files to save storage cost after overwriting the table, you can use [VACUUM](/latest/delta-utility#remove-files-no-longer-referenced-by-a-delta-table) to delete them. It's optimized for file deletion and usually faster than deleting the entire directory.
